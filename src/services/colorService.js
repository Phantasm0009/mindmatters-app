...
export const getMoodColorPalette = async (mood) => {
  try {
    // Enhanced color seeds using color psychology principles
    // Each emotion has carefully selected colors for better emotional resonance
    const moodColorSeeds = {
      happy: [[255, 195, 0], "N", "N", "N", "N"],     // Bright yellow - joy, optimism
      excited: [[255, 89, 94], "N", "N", "N", "N"],   // Coral red - energy, enthusiasm
      calm: [[86, 161, 200], "N", "N", "N", "N"],     // Sky blue - tranquility, peace
      relaxed: [[106, 177, 135], "N", "N", "N", "N"], // Soft green - restoration, harmony
      neutral: [[190, 190, 190], "N", "N", "N", "N"], // Medium gray - balance, stability
      sad: [[108, 117, 187], "N", "N", "N", "N"],     // Muted blue - melancholy, depth
      stressed: [[242, 103, 34], "N", "N", "N", "N"], // Burnt orange - tension, alertness
      angry: [[201, 42, 42], "N", "N", "N", "N"],     // Deep red - intensity, passion
      anxious: [[255, 177, 0], "N", "N", "N", "N"],   // Amber - caution, unease
      tired: [[147, 112, 170], "N", "N", "N", "N"],   // Lavender - fatigue, restfulness
      grateful: [[41, 166, 126], "N", "N", "N", "N"], // Emerald - gratitude, growth
      default: [[66, 133, 244], "N", "N", "N", "N"]   // Google blue - neutral, professional
    };
    
    // More nuanced mapping of numeric mood values (1-10) to mood words
    const moodValueToName = (value) => {
      if (value <= 2) return 'sad';
      if (value === 3) return 'anxious';
      if (value === 4) return 'stressed';
      if (value === 5) return 'neutral';
      if (value === 6) return 'tired';
    ...
    // If mood is a number, convert it to a mood name
    if (!isNaN(mood)) {
      mood = moodValueToName(parseInt(mood));
    }
    
    // Normalize mood name
    mood = mood.toLowerCase();
    
    // Select seed colors based on mood, or use default
    const seedColors = moodColorSeeds[mood] || moodColorSeeds.default;
    
    // Check if online
    if (!navigator.onLine) {
      throw new Error('Offline mode: Using cached color palette');
    }
    
    // Request to Colormind API
    ...
    localStorage.setItem(`colorPalette_${mood}`, JSON.stringify(paletteCache));
    
    return palette;
  } catch (error) {
    console.warn('Failed to fetch color palette:', error);
    
    // Try to use cached palette
    const cachedPaletteJson = localStorage.getItem(`colorPalette_${mood}`);
    if (cachedPaletteJson) {
      const cachedPalette = JSON.parse(cachedPaletteJson);
      return cachedPalette.palette;
    }
    
    // If all fails, return a default palette
